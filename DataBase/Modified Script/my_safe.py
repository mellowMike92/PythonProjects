...
def delete_recovered_files():
    ...
    if confirm_delete.lower().startswith("y"):  # In case caps-lock was on
        program_files = ["safe.py", "mySafe.db"]  # Ensures the main files (script and database) are not deleted.
...
def recover_file():
    file_, cursor, file_name, file_type = create_sql_command()
    ...
    file_string = _collect_binary_data(cursor)
    if file_type == "mp4" or file_type == "mp3":
        recover_audio_video_file(file_, file_name, file_string, file_type)
    else:
        ...
        recover_file_other(file_, file_string)
...
def create_sql_command():
    ...
    return file_, cursor, file_name, file_type


def _collect_binary_data(cursor):
    file_string = ""
    for row in cursor:
        file_string += row[4]
...
def recover_file_other(file_, file_string):
    with open(file_, 'wb') as f_output:
        f_output.write(base64.b64decode(file_string))


def recover_audio_video_file(file_, file_name, file_string, file_type):
...
def delete_database_file():
    ...
    delete_file = "DELETE FROM SAFE WHERE FILE_NAME = " + '"' + path + '"'
    cursor.execute(delete_file)
    conn.commit()


def delete_all_database_files():
    conn = connect_to_database()
    ...
    if path.lower() == "y":  # Perform double-check to confirm deletion of all files.
        ...
        delete_file = "DELETE FROM SAFE"
        cursor.execute(delete_file)
        conn.commit()


def show_database_directory():
    conn = connect_to_database()
...
def run_program():
    ...
    password = "password"
    check_pw = check_input_pw(password)
    ...
    if check_pw == password:
        ...
        while True:
            ...
            elif input_.lower() == "rf":
                recover_file()

            elif input_.lower() == "rd":
                # Recovers a directory by checking the database for the input placed by the user.
                recovered_files = recover_directory_files()
                for file in recovered_files:
                    recover_file()

            elif input_.lower() == "sd":
                store_directory()

            elif input_.lower() == "sf":
                store_file()

            elif input_ == "dr":
                delete_recovered_files()

            elif input_ == "df":
                delete_database_file()

            elif input_ == "da":
                delete_all_database_files()

            elif input_ == "ls":
                show_database_directory()


def check_input_pw(password):
    # Creating a name/variable defined as a password entry (without echoing the input)
    check_pw = getpass.getpass(prompt="\nPlease enter your password:\n")
