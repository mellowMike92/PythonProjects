...
class Main(DataBase):
    ...
    def create_password_prompt(self):
        ...
        if pw_prompt.lower() == "y":
            self.start_password_creation()
        elif pw_prompt.lower() == "n":
            return
    ...
    def start_password_creation(self):
        password_creation = input("Would you like to create a new random password?\t(Y\\N)\n")
        if password_creation.lower() == "y":
            create_random_pw_length = input("Specify password length and hit Enter (12 is recommended)\n").strip()
            while not create_random_pw_length.isnumeric():
                create_random_pw_length = input("Specify password length and hit Enter (12 is recommended)\n")
            self.create_random_password(int(create_random_pw_length))

        elif password_creation.lower() == "n":
